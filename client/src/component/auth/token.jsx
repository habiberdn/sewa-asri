...
export default function NewPasswrod() {
  ...
  const handleSubmit = async (e) => {
    ...
    console.log(token, data.data.data.doc.passwordResetToken)
    console.log(new Date(Date.now()), data.data.data.doc.passwordResetExpires)
    if (new Date(Date.now()) < new Date(data.data.data.doc.passwordResetExpires )&& data.data.data.doc.passwordResetToken === token) {
